...
class BeikeSpider(BaseSpider):
    ...
    def get_house_list(self, district: str, page: int = 1) -> List[Dict]:
        ...
        house_items = soup.select('ul.sellListContent li') or soup.select('div.listContent li')
    ...
    def _parse_house_item(self, item, district: str) -> Optional[Dict]:
        try:
            title_elem = item.select_one('div.title a') or item.select_one('a.clickarea')
            if not title_elem:
                return None
            ...
            position_elem = item.select_one('div.positionInfo') or item.select_one('div.flood')
            position = self.clean_text(position_elem.text) if position_elem else ""

            house_info_elem = item.select_one('div.houseInfo') or item.select_one('div.houseInfo')
            house_info = self.clean_text(house_info_elem.text) if house_info_elem else ""

            price_elem = item.select_one('div.totalPrice span') or item.select_one('span.totalPrice')
            price_text = price_elem.text if price_elem else "0"
            price = self.parse_price(price_text)

            unit_price_elem = item.select_one('div.unitPrice span') or item.select_one('span.unitPrice')
            unit_price = self.clean_text(unit_price_elem.text) if unit_price_elem else ""

            follow_elem = item.select_one('div.followInfo') or item.select_one('div.star')
            follow_info = self.clean_text(follow_elem.text) if follow_elem else ""

            tag_elems = item.select('div.tag span') or item.select('span.tag')
    ...
    def get_house_detail(self, house_url: str) -> Dict:
        ...
        try:
            ...
            base_info = soup.select('div.base li') or soup.select('ul.baseList li')
            ...
            transaction_info = soup.select('div.transaction li') or soup.select('ul.transaction li')
            ...
            img_elems = soup.select('div.imgList img') or soup.select('div.img-box img')
